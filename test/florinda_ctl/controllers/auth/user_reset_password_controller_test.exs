defmodule FlorindaCtl.Auth.UserResetPasswordControllerTest do
  ...
  describe "GET /auth/users/reset_password" do
  ...
  describe "POST /auth/users/reset_password" do
    @tag :capture_log
  ...
  describe "GET /auth/users/reset_password/:token" do
    setup %{user: user} do
      token =
  ...
  describe "PUT /auth/users/reset_password/:token" do
    setup %{user: user} do
      token =
        extract_user_token(fn url ->
          Accounts.deliver_user_reset_password_instructions(user, url)
